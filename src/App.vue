...
function addItem() {
  ...
  // 할일 추가후 입력필드와 연결된 데이터를 빈문자으로 초기화
  data.newItem = '';
}
// 할일 제거
...
<template>
  <main class="app">
    ...
    <ul class="todo_list">
      ...
      <!-- todo.completed가 true면 completed 문자열이 글래스명이됨 -->
      <li
        v-for="(todo, i) in data.items"
        v-bind:key="todo.id"
        v-bind:class="{ completed: todo.completed }"
      >
        <!-- id가 달라야하므로 id속성과 todo.id값을 단방향으로 연결 -->
        <input
          type="checkbox"
